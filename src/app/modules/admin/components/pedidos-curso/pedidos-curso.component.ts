...
export class PedidosCursoComponent implements OnInit {
  ...
  open(content, key:string) {
    ...
    this.pedidosEditar = [];
    this.firestore.collection('pedidosCurso').doc(key).valueChanges()
    .subscribe(val => {
    ...
    this.modalService.open(content, {ariaLabelledBy: 'modal-basic-title'}).result.then((result) => {
     ...
     this.firestore.collection('pedidosCurso').doc(key).update({pedidos:this.pedidosEditar[0]['pedidos'], cantidad: Number(this.pedidosEditar[0]['cantidad'])})       
  ...
  openAgregar(content, key:string) {
    ...
    this.firestore.collection('pedidosCurso').doc(key).valueChanges().pipe(take(1)).subscribe(val => {

      let valor = val['pedidos']
      valor.push(this.agregarPedido)

      this.firestore.collection('pedidosCurso').doc(key).update({pedidos:valor}) 
  ...
  editarPedidos(model: string, valorItem: string, e) {
    ...
    switch(model) {
      ...
      case 'estadoAlmacen':
          ...
          break;
      case 'estadoOficina':
          this.pedidosEditar[0]['estadoOficina'][model] = e.target.value;
          break;
    }
